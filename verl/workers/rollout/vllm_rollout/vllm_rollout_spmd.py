...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            # 调试：打印输入列表长度和第一个元素详情
            print(f"[vllm_rollout] vllm_inputs len={len(vllm_inputs)}")
            if vllm_inputs:
                first = vllm_inputs[0]
                print("[vllm_rollout] vllm_inputs[0] keys:", list(first.keys()))
            ...
            if outputs:
                out0 = outputs[0]
                print("[vllm_rollout] outputs[0] attrs:", out0.__dict__.keys())
                print(f"  request_id={out0.request_id}, finished={getattr(out0, 'finished', None)}")
                if out0.outputs:
                    sample0 = out0.outputs[0]
                    print("  first sample fields:",
                          f"token_ids len={len(sample0.token_ids)}",
                          f"text={sample0.text[:80]}...")
